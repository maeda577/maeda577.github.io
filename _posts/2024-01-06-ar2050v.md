...
# 変なファイルとかが無いことを見る あれば消す
show file systems
dir flash:
delete flash:hogehoge.txt
# 再起動
reload
```
...
zone public
 network wan
  ...
  host eth1
   # この書き方で外部インターフェースのことを指定できるらしい ルーター発の通信を許可する時に使う
...
interface eth1
 # IPv4はDHCP
 ip address dhcp
 # IPv6はRouterAdvertisementを見るSLAACを使う
...
DNS設定
------------------------
* DNSサーバはDHCP経由で降ってきたりするので、そもそも指定が不要だったりする

``` shell
# ルーター自身が使うDNSサーバを指定する IPv6アドレスでも良い
ip name-server 8.8.8.8
ip name-server 8.8.4.4
# ルーターをDNSキャッシュサーバとして動かす 要らなければ切っておいた方が良い
ip dns forwarding
```
